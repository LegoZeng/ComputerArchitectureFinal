...
namespace sepgraph
{
    namespace engine
    {
        ...
        public:
            Engine(AlgoType algo_type) : m_running_info(algo_type),
                                         m_policy_decision_maker(m_running_info)
            {
                int dev_id = 0;

                GROUTE_CUDA_CHECK(cudaGetDeviceProperties(&m_dev_props, dev_id));
                m_groute_context = std::unique_ptr<utils::traversal::Context<Algo>>(new utils::traversal::Context<Algo>(1));
            ...
            void InitGraph(UnusedData &...data)
            {
                ...
                kernel::InitGraph<<<grid_dims, block_dims, 0, stream.cuda_stream>>>(app_inst,
                                                                                    work_source,
                ...
                kernel::RebuildWorklist<<<grid_dims, block_dims, 0, stream.cuda_stream>>>(app_inst,
                                                                                          work_source,
                                                                                          graph_datum.m_wl_array_in.DeviceObject(),
                                                                                          graph_datum.GetBufferDeviceObject());
                stream.Sync();

                kernel::InitDegree<<<grid_dims, block_dims, 0, stream.cuda_stream>>>(dev_csr_graph,
                                                                                     work_source,
                ...
                if (FLAGS_trace || !m_engine_options.IsForceVariant())
                {
                    ...
                    if (algo_variant.GetScheduling() == Scheduling::DATA_DRIVEN)
                    {
                        if (algo_variant.m_msg_passing == MsgPassing::PUSH)
                        {
                            ...
                            kernel::SumOutDegreeQueue<<<grid_dims, block_dims, 0, stream.cuda_stream>>>(groute::dev::WorkSourceArray<index_t>(
            ...
            void ExecutePolicy(const AlgoVariant &algo_variant)
            {
                ...
                else if (algo_variant == AlgoVariant::ASYNC_PUSH_DD)
                {
                    if (m_engine_options.IsFused())
                    {
                        if (m_engine_options.GetPriorityType() == Priority::LOW_HIGH)
                        {
                            if (m_running_info.current_round % 100 == 0)
                            {
                                current_priority *= 1.1;
                                m_engine_options.SetTwoLevelBasedPriority(current_priority);
                            }
                ...
                if (FLAGS_trace || !m_engine_options.IsForceVariant())
                {
                    ...
                    else
                    {
                        ...
                        if (FLAGS_trace)
                        {
                            ...
                            kernel::SumOutDegreeBitmap<<<grid_dims, block_dims, 0, stream.cuda_stream>>>(graph_datum.m_wl_bitmap_in.DeviceObject(),
                                                                                                         graph_datum.m_out_degree.dev_ptr,
                                                                                                         graph_datum.m_total_out_degree.dev_ptr);
                            stream.Sync();
