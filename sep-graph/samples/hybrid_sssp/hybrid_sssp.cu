...
namespace hybrid_sssp
{
    ...
    struct SSSP : sepgraph::api::AppBase<TValue, TBuffer, TWeight>
    {
        ...
        __forceinline__ __device__
            ...
            bool
            IsActiveNode(index_t node, TBuffer buffer) const override
        {
        ...
        __forceinline__ __device__
            ...
            bool
            IsHighPriority(TBuffer current_priority, TBuffer buffer) const override
        {
...
bool HybridSSSP()
{
     ...
     */
    int init_prio = 2 * 32 * (weight_sum / csr_graph.nedges) /
